...
# Collect all resources for key libraries
...
a = Analysis(
    ...
    datas=[('fonts', 'fonts'),
           ('icon.png', '.'),
           ('urdu_render.py', '.'), # CRITICAL: Missing this caused Urdu fonts to fail
    ...
    hiddenimports=(rembg_hidden + ort_hidden + sp_hidden + pkg_hidden + hb_hidden + ft_hidden +
                   ['rembg', 'rembg.bg', 'rembg.sessions', 'rembg.session_factory',
                    ...
                    'setuptools', 'pkg_resources', 'urdu_render', 'uharfbuzz', 'freetype']),
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
    excludes=[],
...
exe = EXE(
    ...
    a.datas,
    [],
    name='QaziUrduDesigner_Pro', # Changed name to ensure fresh icon and cache
    debug=False,
    bootloader_ignore_signals=False,
    strip=False,
    upx=False, # DISABLED UPX for fast opening
    upx_exclude=[],
    runtime_tmpdir=None,
    console=False,
